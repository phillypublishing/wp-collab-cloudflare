...
2. The **plugin** uses the [`sync.providers`](https://developer.wordpress.org/reference/hooks/sync-providers/) filter to replace WordPress's default HTTP polling provider with a WebSocket provider. Before connecting, it requests a short-lived credential through Core `wp.apiFetch`, including its supported cookie/nonce refresh behavior. WordPress verifies `edit_post` for a post. Collection requests keep Gutenberg's native `objectId: null`, require the HTTP sync server's minimum `edit_posts` capability, and use `WP_Sync_Config` plus the `wp_collab_cf_collection_sync_permission` extension filter. The provider uses `y-partyserver` and shares the editor's Yjs module through the `Y` provider option, with a fallback to `wp.sync.Y` on older Gutenberg versions. The build aliases Yjs imports to a runtime shim to avoid bundling a second copy; `wp-sync` is required only when registered.
