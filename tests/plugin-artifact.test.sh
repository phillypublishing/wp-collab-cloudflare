...
export PLUGIN_ARTIFACT_SOURCE_SHA="${PLUGIN_ARTIFACT_SOURCE_SHA:-0000000000000000000000000000000000000000}"
export SOURCE_DATE_EPOCH="${SOURCE_DATE_EPOCH:-1770000000}"

first_output_dir="${1:-${test_root}/first}"
"${repo_root}/scripts/build-plugin-artifact.sh" "${first_output_dir}"
"${repo_root}/scripts/build-plugin-artifact.sh" "${test_root}/second"

first_zip="$(find "${first_output_dir}" -maxdepth 1 -type f -name '*.zip' -print -quit)"
...
' "${first_output_dir}/plugin-artifact-manifest.json" "${expected_entries}"
